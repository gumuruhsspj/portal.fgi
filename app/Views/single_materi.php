          ...
          <div class="card card-solid">
              ...
              <div class="card-body">
                <div class="row">
                  ...
                  <div class="col-12 col-sm-6">
                    ...
                    <?php if (empty($paket_ada)): ?>

                      <div class="alert alert-warning mb-2">
                        <i class="fas fa-info-circle mr-1"></i>
                        Paket untuk materi ini <strong>belum tersedia</strong>. Sedang dipersiapkan.
                      </div>
                    ...
                    <?php else: ?>
                      ...
                      <div class="btn-group btn-group-toggle" data-toggle="buttons">
                        ...
                        <?php if (in_array('paket_belajar_sendiri', $paket_tersedia)): ?>
                          <label class="btn btn-default text-center <?= $default_paket == 'paket_belajar_sendiri' ? 'active' : '' ?>">
                            <input class="btn_opsi_paket" type="radio" name="color_option"
                              data-biaya="<?= $data_materi->biaya_belajar_sendiri; ?>"
                              id="paket_belajar_sendiri" autocomplete="off"
                              value="paket_belajar_sendiri"
                              <?= $default_paket == 'paket_belajar_sendiri' ? 'checked' : '' ?>>
                            <i class="fas fa-book fa-2x"></i> <br>
                            Belajar Sendiri + Materi (Video + Modul)
                          </label>
                        <?php endif; ?>

                        <?php if (in_array('paket_bimbingan', $paket_tersedia)): ?>
                          <label class="btn btn-default text-center <?= $default_paket == 'paket_bimbingan' ? 'active' : '' ?>">
                            <input class="btn_opsi_paket" type="radio" name="color_option"
                              data-biaya="<?= $data_materi->biaya_pokok; ?>"
                              id="paket_bimbingan" autocomplete="off"
                              value="paket_bimbingan"
                              <?= $default_paket == 'paket_bimbingan' ? 'checked' : '' ?>>
                            <i class="fas fa-users fa-2x"></i> <br>
                            Bimbingan (Live) + Materi (Video + Modul)
                          </label>
                        <?php endif; ?>

                        <?php if (in_array('paket_kasus_custom', $paket_tersedia)): ?>
                          <label class="btn btn-default text-center <?= $default_paket == 'paket_kasus_custom' ? 'active' : '' ?>">
                            <input class="btn_opsi_paket" type="radio" name="color_option"
                              data-biaya="<?= $data_materi->biaya_kasus_custom; ?>"
                              id="paket_kasus_custom" autocomplete="off"
                              value="paket_kasus_custom"
                              <?= $default_paket == 'paket_kasus_custom' ? 'checked' : '' ?>>
                      ...
                      <div class="bg-gray py-2 px-3 mt-4">
                        <h2 id="materi-harga" class="mb-0">
                          <?= as_rupiah($default_biaya + (0.1 * $default_biaya)); ?>
                        </h2>
                        <h4 class="mt-0">
                          <small>Sudah Termasuk Pajak 10%:
                            <span id="biayapajak"><?= as_rupiah(0.1 * $default_biaya) ?></span>
                          </small>
                          <br>
                    ...
                    <div class="mt-4">
                      <?php if (empty($participate) && empty($paket_ada)): ?>
                        <!-- Kalau belum punya paket & paket belum tersedia: tombol disabled -->
                        <div class="btn btn-secondary btn-lg btn-flat disabled"
                          style="cursor: not-allowed; pointer-events: auto; opacity: .65;"
                          title="Paket belum tersedia">
                          <i class="fas fa-ban fa-lg mr-2"></i>
                          Belum Tersedia
                        </div>
                      <?php else: ?>
                        <div data-status="<?= empty($participate) ? 'none' : 'subscribed' ?>"
                          class="btn btn-primary btn-lg btn-flat btn-pilih-materi"
            ...
            <?php else : ?>
              <div class="alert alert-warning">
                Materi tidak ditemukan. Silakan kembali ke daftar materi.
              </div>
            <?php endif; ?>
            <!-- /.card-body -->
          </div>
